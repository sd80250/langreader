...
def compare(index1, index2, print_=False):
    text1_fv = v.make_relative(v.frequency_vector(index_list[index1][0]))
    text2_fv = v.make_relative(v.frequency_vector(index_list[index2][0]))
    if print_:
...
def test(poem_path, model_path=None):
    ...
    if model_path:
        poems = pd.DataFrame(pd.read_csv(
            'PoetryFoundationData.csv'), columns=['Poem', 'Title'])
...
if not path.exists('models/global_vector.p'):
    v.make_global_vector()
if not path.exists('models/svm_model563.p'):
    svm.make_and_test_model(563)
if not path.exists('poems.p'):
    df = pd.DataFrame(pd.read_csv('PoetryFoundationData.csv'), columns=['Poem', 'Title'])
    init_variables(list(df.to_records(index=False))[0:100])
    sorted_poems = [index_list[i] for i in init_sort(list(range(0, 100)), k_max=2)]
    pickle.dump(sorted_poems, open('poems.p', 'wb'))
App('poems.p')
