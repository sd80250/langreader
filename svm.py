...
def make_and_test_model(pairs_of_test_data, name='models/svm_model.p', samples=1000):
    # get training data
    ...
    try:
        for i in range(0, n_iters):
            ...
            for X_train, y_train, X_test, y_test in vectorizer.make_test_and_training_data(data_per_batch_times_two):
                ...
                process = psutil.Process(os.getpid())
                print("done; memory =", process.memory_info().rss // 1000000, "MB; score =", accuracy, "; amount of data trained:", batch_number*data_per_batch_times_two*(1-test_size) ,"; estimated time left: ", "%02d:%02d:%02d" % (completion_time // 3600, (completion_time % 3600 // 60), (completion_time % 60 // 1)), end="\n\r")
